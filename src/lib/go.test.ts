...
describe("targetForSlug", () => {
  it("resolves a known solution slug to its product url", () => {
    expect(targetForSlug("epistemix")).toBe("https://epistemix.dev");
  });
...
describe("handleGo", () => {
  const target = "https://epistemix.dev";

  it("302-redirects a known slug to its target and records the click", async () => {
    const record = vi.fn(async () => {});
    const res = await handleGo("epistemix", { resolve: () => target, record });
    expect(res.status).toBe(302);
    expect(res.headers.get("location")).toBe(target);
    expect(record).toHaveBeenCalledWith("epistemix");
  });
