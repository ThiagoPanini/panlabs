...
3. **Catálogo V1.0 = 2 soluções reais + 1 placeholder.** ethitorial (`live`) e travelmanager (`beta`) redirecionam de fato. O 3º card é um **placeholder anônimo `// TODO`** — sem link, sem stats, sem CTA, preview vazio, zero motion. É o lugar do estágio "em breve" sem renderizar badge. **querymind não é real**; entra via PR ([ADR-0006](0006-catalogo-tres-fontes.md)) quando existir.
